...
- `notify(value: string | number | boolean | undefined | null)`: immediately injects an extra `custom_tool_call_output` for the current `exec` call. If compaction has removed that call, the model receives a bounded `<code_mode_notification>` message instead. Values are stringified like `text(...)`.
