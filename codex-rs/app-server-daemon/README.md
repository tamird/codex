...
The legacy `managedCodexPath` and `managedCodexVersion` fields describe the
selected executable, including externally managed installations.
...
`bootstrap` records the daemon settings under `CODEX_HOME/app-server-daemon/`
and starts app-server as a pidfile-backed detached process. Only standalone
managed installations launch a detached updater loop.

## Installation and update cases

Standalone installations launch the managed current binary under `CODEX_HOME`.
Other installations launch the invoking executable, even when a standalone
installation also exists. Installation provenance, not file existence, selects
who owns updates.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| An external tool or custom build installed Codex | The invoking executable | No, including after `bootstrap` or `remote-control` | No. Update with the original tool, then explicitly restart. |
| A standalone CLI invokes `start` | `start` uses `CODEX_HOME/packages/standalone/current/codex` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| A standalone CLI invokes `bootstrap` | The pidfile backend uses `CODEX_HOME/packages/standalone/current/codex` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
Custom builds and package-manager installations support the same lifecycle
commands, but `bootstrap` reports `autoUpdateEnabled: false`. Neither bootstrap
nor remote control installs or runs the standalone updater for them.

There is one daemon per `CODEX_HOME`. Its PID record identifies the installation
that launched it. Implicit local attachment, start, restart, and
reconfiguration reject a different recorded installation. Explicit `stop`
still stops this home's daemon, allowing a deliberate installation switch.
External clients also reject legacy records or
unowned sockets; `--remote` remains an explicit connection choice. Updating an
external executable at the same path takes effect on restart. If an update
changes its path, explicitly stop the old daemon before starting the new one.

A live standalone updater prevents an external daemon from starting in that
home, even if the daemon itself is stopped. `daemon stop` does not stop the
updater. Use a separate `CODEX_HOME` when both installations need to coexist.
Older clients do not enforce executable ownership, so a separate home is also
required for isolation from unpatched clients.
...
`restart` stops this installation's daemon and starts it again.
...
For standalone installations, `codex remote-control start` bootstraps with
`--remote-control` when the updater loop is not running. Otherwise it enables
remote control and starts the daemon normally. External installations always
use the latter flow without an updater.
