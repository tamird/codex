...
- `rolloutMaintenance/status/changed` — experimental; sends `type: "snapshot"` on initialization and local automatic-migration changes. The snapshot's lock state is sampled, not a subscription to every other process: use status/read for a fresh observation. Phase counts describe only that phase, and `ioBytes` counts observed bytes handled, including repeated passes. Status does not add thread-history items.
