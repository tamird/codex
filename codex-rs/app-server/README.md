...
- `thread/list` — page through stored threads; supports cursor-based pagination and optional `modelProviders`, `sourceKinds`, `archived`, `sectionId`, `cwd`, and `searchTerm` filters. Experimental `projectId` filters one project, while `null` selects unassigned threads. Set `sortKey` to `"section_position"` when listing a section in its persisted manual order. Experimental clients can use `parentThreadId` for direct current-agent children or `ancestorThreadId` for current descendants at any depth; the two filters are mutually exclusive. Both queries use the loaded root thread's current-agent membership. An unloaded root returns no current members; persisted spawn edges and historical transcript activity alone do not create membership. When `archived` is omitted from these relationship-filtered requests, both archive states are included. Review and Guardian threads are not included in these membership queries. Each returned `thread` includes `status` (`ThreadStatus`), defaulting to `notLoaded` when the thread is not currently loaded. Subagent threads also include `parentThreadId` when the immediate parent is known.
...
- `rolloutMaintenance/status/read` — experimental; return a `status` snapshot containing the cross-process rollout-maintenance `lock` (`idle` or `busy`, with nullable owner details) and this server's `backgroundMigration` activity. A running migration can release the lock before finishing projection work, so these fields are independent. `backgroundMigration: null` means the thread store does not expose an automatic migration worker.
- `rolloutMaintenance/status/changed` — experimental; sends `type: "snapshot"` on initialization and local automatic-migration changes, or `type: "request"` with the originating connection's `requestId` and request-local status. `waitingForMaintenance` describes actual contention, not merely a busy global lock. Its owner details are refreshed by the storage wait; phase counts describe only that phase, and `ioBytes` counts observed bytes handled, including repeated passes. The snapshot's lock state is sampled, not a subscription to every other process: use status/read for a fresh observation. Clients should display local `backgroundMigration` independently and show an external owner only for a matching pending request. Request updates are advisory, may be coalesced or dropped, and are never enqueued after that request's final response/error. That response/error or a disconnect clears request status; a separate final `idle` update is not required. Status does not add thread-history items.
...
### Example: List current agent descendants

Enable `capabilities.experimentalApi` during initialization, then use `thread/list` with `ancestorThreadId` to page through the current descendants of a loaded thread. The ancestor itself is excluded, and each result's `parentThreadId` remains its immediate parent. Use `parentThreadId` instead to list only direct current-agent children; grandchildren are excluded from that direct-child query. These queries use current-agent membership, so an unloaded root has no current members and persisted spawn edges or historical transcript activity alone do not add results. Sending both filters is invalid. Review and Guardian threads are not included in these membership queries. Omitting `archived` includes both archive states for relationship-filtered requests; ordinary thread lists still default to non-archived threads. When `modelProviders` or `sourceKinds` is omitted, relationship-filtered requests include every provider or source kind, respectively. Explicit filters retain the ordinary `thread/list` behavior, including the interactive-only default for an empty `sourceKinds` list.
